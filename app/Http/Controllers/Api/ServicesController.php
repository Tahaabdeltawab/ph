<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Http\Resources\ServicesResource;
use App\Models\Place_service;
use Validator,Auth,Artisan,Hash,File,Crypt;
use carbon\Carbon;

class ServicesController extends Controller
{
    use ApiResponseTrait;

    public function update_services(Request $request){

        $lang = $request->header('lang');

        if(auth()->User()){
            $created_at = carbon::now()->toDateTimeString();
            $dateTime = date('Y-m-d H:i:s',strtotime('+2 hours',strtotime($created_at)));

            $service_id = $request->service_id;

            $data = Place_service::where('id' , $service_id)->first();

            $check = $this->not_found($data , 'الخدمة', 'Service' , $lang);
            if( isset($check) ){
                return $check;
            }
        
            $details_en = $request->has('details_en') && $request->details_en != NULL ? $request->details_en : $data->details_en;
            $details_ar = $request->has('details_ar') && $request->details_ar != NULL ? $request->details_ar : $data->details_ar;
            
            $update_service = Place_service::where('id' , $service_id)->update([
                                                                            "details_en" => $details_en,
                                                                            "details_ar" => $details_ar,
                                                                            "updated_at" => $dateTime
                                                                            ]);

            $msg = $lang == 'ar' ? " تم تعديل الخدمة بنجاح" : "Service is updated";

            return $this->apiResponseMessage( 0, $msg);
        }else{
            $msg = $lang == 'ar' ? " من فضلك سجل دخول" : "Token is not provided";

            return $this->apiResponseMessage( 3, $msg);
        }
    }


    public function add_services(Request $request){
        $lang = $request->header('lang');

        if(auth()->User()){
            $created_at = carbon::now()->toDateTimeString();
            $dateTime = date('Y-m-d H:i:s',strtotime('+2 hours',strtotime($created_at)));

            $input = $request->all();
            $validationMessages = [
                'details_ar.required' => $lang == 'ar' ?  'من فضلك ادخل تفاصيل الخدمة باللغه العربية' :"please enter service details in arabic" ,        
                'details_en.required' => $lang == 'ar' ?  'من فضلك ادخل تفاصيل الخدمة باللغه الانحليزية' :"please enter service details in english" ,      
            ];

            $validator = Validator::make($input, [
                'details_en' => 'required',
                'details_ar' => 'required',
            ], $validationMessages);

            if ($validator->fails()) {
                return $this->apiResponseMessage(1,$validator->messages()->first(), 200);
            }

            $add = new Place_service;
            $add->details_ar = $request->details_ar;
            $add->details_en = $request->details_en;
            $add->place_id = $request->place_id;
            $add->created_at = $dateTime;
            $add->updated_at = $dateTime;
            $add->save();
          
            $msg = $lang == 'ar' ? " تم أضافه خدمة جديده" : "new service is inserted";

            if($request->header('src') == 'web')
            return $this->apiResponseData( new ServicesResource($add), $msg);
            return $this->apiResponseMessage( 0, $msg);

        }else{
            $msg = $lang == 'ar' ? " من فضلك سجل دخول" : "Token is not provided";

            return $this->apiResponseMessage( 3, $msg);
        }
    }

    public function delete_service(Request $request){
        $lang = $request->header('lang');

        if(auth()->User()){
            $service_id = $request->service_id;

            $data = Place_service::where('id' , $service_id)->first();

            $check = $this->not_found($data , 'الخدمة', 'Service' , $lang);
            if( isset($check) ){
                return $check;
            }
         
            $delete_service = Place_service::where('id' , $service_id)->delete();

            $msg = $lang == 'ar' ? "تم مسح الخدمه بنجاح" : "Service is deleted";

            return $this->apiResponseMessage( 0, $msg);

        }else{
            $msg = $lang == 'ar' ? " من فضلك سجل دخول" : "Token is not provided";

            return $this->apiResponseMessage( 3, $msg);
        }
    }
}
